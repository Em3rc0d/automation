...
const wfOut=path.join(os.tmpdir(),'case003-g8-disarm-send-'+process.pid+'.json');
const credOut=path.join(os.tmpdir(),'case003-g8-disarm-api-'+process.pid+'.json');
...
(async()=>{
  try{
    ...
    if(!case002?.id||case002.active!==true) throw new Error('CASE-002 webhook missing/inactive; refusing cleanup');
    if(!gate8?.id) throw new Error('CASE-003 Gate-8 webhook missing');

    if(gate8.active!==false){
      const patch=await req(base+'/'+encodeURIComponent(gate8.id),{
        method:'PATCH',
        headers:{...headers,'Content-Type':'application/json'},
        body:JSON.stringify({whatsapp_webhook:{active:false}})
      });
      if(!patch.ok) throw new Error('deactivate Gate-8 webhook failed HTTP '+patch.status+' '+JSON.stringify(patch.body).slice(0,300));
    }
    ...
    if(!after.ok) throw new Error('post-cleanup list failed HTTP '+after.status);
    ...
    if(gate8After?.active!==false) throw new Error('CASE-003 webhook not inactive after cleanup');

    console.log('[case003-gate8-disarm] case002Webhook='+JSON.stringify(safeHook(case002After)));
    console.log('[case003-gate8-disarm] gate8Webhook='+JSON.stringify(safeHook(gate8After)));
    console.log('[case003-gate8-disarm] PASS gate8WebhookActive=false gate7WorkflowActive=false case002Unchanged=true');
  } finally {
    try{fs.unlinkSync(wfOut);}catch(_){}
    try{fs.unlinkSync(credOut);}catch(_){}
  }
})().catch(e=>{console.error('[case003-gate8-disarm] FAIL '+(e.stack||e.message));process.exit(1);});
