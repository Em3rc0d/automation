...
function run(db, sql, params = []) {
  return new Promise((resolve, reject) => db.run(sql, params, function (e) { e ? reject(e) : resolve({ changes: this.changes }); }));
}
function close(db) {
...
async function inspectCandidate(file) {
  try {
    ...
    await close(db);
    return { file, valid, workflows, users, size: fs.statSync(file).size };
  } catch (e) {
    return { file, valid: false, workflows: -1, users: -1, size: 0, error: e?.message || String(e) };
  }
}
...
async function locateDatabase() {
  ...
  const explicit = configured
    ? (path.isAbsolute(configured) ? configured : path.join(n8nBase, '.n8n', configured))
    : null;
  ...
  for (const file of candidates) {
    ...
    inspected.push(info);
    log(`candidate: ${file} valid=${info.valid} workflows=${info.workflows} users=${info.users} bytes=${info.size}`);
  }

  const valid = inspected.filter((x) => x.valid);
  if (!valid.length) return null;
  valid.sort((a, b) => (b.workflows - a.workflows) || (b.size - a.size));
  return valid[0];
}
...
async function main() {
  const selected = await locateDatabase();
  if (!selected) {
  ...
  try {
    ...
    report.preflight = { workflowCount, sharedCount, projectCount, relationCount, users: users.length, owners: owners.length };
    ...
    if (workflowCount === 0) {
      report.status = 'aborted-zero-workflows'; writeReport();
    ...
    const ownerRelations = await all(db, `SELECT "projectId" AS projectId, role FROM project_relation WHERE "userId"=?`, [ownerId]);
    const accessible = new Set(ownerRelations.map((r) => r.projectId));

    let personal = await all(db, `SELECT p.id, p.name, p.type, p."creatorId" AS creatorId FROM project p JOIN project_relation pr ON pr."projectId"=p.id WHERE pr."userId"=? AND p.type='personal'`, [ownerId]);
    let target = personal[0] || null;
    let needsTargetRelation = false;
    if (!target) {
      const byCreator = await all(db, `SELECT id, name, type, "creatorId" AS creatorId FROM project WHERE type='personal' AND "creatorId"=?`, [ownerId]);
      if (byCreator.length === 1) { target = byCreator[0]; needsTargetRelation = true; }
      else {
        const allPersonal = await all(db, `SELECT id, name, type, "creatorId" AS creatorId FROM project WHERE type='personal'`);
    ...
    if (needsTargetRelation && !accessible.has(target.id)) {
      plans.push({ projectId: target.id, role: 'project:personalOwner', reason: 'restore-owner-personal-project' });
    }

    const workflowProjects = await all(db, `SELECT DISTINCT sw."projectId" AS projectId, p.type FROM shared_workflow sw JOIN project p ON p.id=sw."projectId"`);
    for (const p of workflowProjects) {
      if (accessible.has(p.projectId) || plans.some((x) => x.projectId === p.projectId)) continue;
      if (p.type === 'personal') {
        plans.push({ projectId: p.projectId, role: 'project:personalOwner', reason: 'restore-workflow-project-access' });
      } else if (p.type === 'team' && roles.has('project:admin')) {
        plans.push({ projectId: p.projectId, role: 'project:admin', reason: 'restore-team-project-access' });
      } else {
    ...
    const withoutOwner = await all(db, `SELECT w.id, w.name FROM workflow_entity w WHERE NOT EXISTS (SELECT 1 FROM shared_workflow sw WHERE sw."workflowId"=w.id AND sw.role='workflow:owner')`);

    await run(db, 'BEGIN IMMEDIATE TRANSACTION');
    try {
      for (const p of plans) {
        const r = await run(db, `INSERT OR IGNORE INTO project_relation ("projectId","userId",role) VALUES (?,?,?)`, [p.projectId, ownerId, p.role]);
        if (r.changes) report.insertedProjectRelations.push(p);
      }
      for (const w of withoutOwner) {
        const existing = await get(db, `SELECT role FROM shared_workflow WHERE "workflowId"=? AND "projectId"=?`, [w.id, target.id]);
        if (existing) {
          const r = await run(db, `UPDATE shared_workflow SET role='workflow:owner', "updatedAt"=STRFTIME('%Y-%m-%d %H:%M:%f','NOW') WHERE "workflowId"=? AND "projectId"=?`, [w.id, target.id]);
          if (r.changes) report.insertedWorkflowOwners.push({ workflowId: w.id, action: 'promoted-existing-share' });
        } else {
          const r = await run(db, `INSERT INTO shared_workflow ("workflowId","projectId",role) VALUES (?,?,'workflow:owner')`, [w.id, target.id]);
          if (r.changes) report.insertedWorkflowOwners.push({ workflowId: w.id, action: 'inserted-owner-share' });
        }
      }

      const visible = Number((await get(db, `SELECT COUNT(DISTINCT w.id) AS count FROM workflow_entity w JOIN shared_workflow sw ON sw."workflowId"=w.id JOIN project_relation pr ON pr."projectId"=sw."projectId" AND pr."userId"=?`, [ownerId])).count || 0);
      const stillNoOwner = Number((await get(db, `SELECT COUNT(*) AS count FROM workflow_entity w WHERE NOT EXISTS (SELECT 1 FROM shared_workflow sw WHERE sw."workflowId"=w.id AND sw.role='workflow:owner')`)).count || 0);
      await run(db, 'COMMIT');
      ...
      report.accessibleWorkflows = await all(db, `SELECT DISTINCT w.id, w.name FROM workflow_entity w JOIN shared_workflow sw ON sw."workflowId"=w.id JOIN project_relation pr ON pr."projectId"=sw."projectId" AND pr."userId"=? ORDER BY w.name`, [ownerId]);
      writeReport();
