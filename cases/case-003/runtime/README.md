...
## Gate 6 — authenticated channel ingress

Gate 6 adds a provider-neutral authenticated webhook in front of Gate 5. The flow is `channel event -> webhook auth -> replay protection -> verified identity or verification initiation -> safe response`.

RUC/tax ID remains identification only. Unknown channel subjects are never auto-bound from a RUC claim. A valid candidate creates only a pending verification request against the pre-existing trusted contact; the contact is masked and delivery remains disabled.
