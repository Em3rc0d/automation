...
if(!['pre','post'].includes(mode)){console.error('usage: verify-gate5-import.js <pre|post>');process.exit(2);}
const targetId='case003SupplierQueryGate5V1';
const credentialId=process.env.CASE003_GATE5_CREDENTIAL_ID||'case003RpcAuthV1';
const checkpoint='/tmp/case003-gate5-portable-pre.json';
...
(async()=>{
 ...
 if(mode==='pre'){
   if(target) throw new Error('Gate-5 target already exists');
   if(!credentials.find(c=>c.id===credentialId)) throw new Error('Gate-5 RPC credential missing: '+credentialId);
   ...
   if(workflows.length!==before.workflowCount+1||credentials.length!==before.credentialCount) throw new Error('Gate-5 count invariant failed');
   ...
   const t=workflows.find(w=>w.id===targetId); if(!t) throw new Error('Gate-5 target missing');
   if(!(t.active===0||t.active===false||t.active==='0')) throw new Error('Gate-5 target must remain inactive');
