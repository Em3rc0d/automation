...
if(!['pre','post'].includes(mode)){console.error('usage: verify-gate7-test-state.js <pre|post>');process.exit(2);}
const targetId='case003KapsoIngressGate7V1';
const checkpoint='/tmp/case003-gate7-test-portable-pre.json';
...
(async()=>{
 ...
 const target=workflows.find(w=>w.id===targetId); if(!target) throw new Error('Gate-7 target missing');
 if(mode==='pre'){
   ...
   console.log('[case003-gate7-test] PRE PASS');
 } else {
   ...
   if(sha(t.nodes)!==before.targetNodesHash||sha(t.connections)!==before.targetConnectionsHash) throw new Error('Gate-7 definition changed');
   console.log('[case003-gate7-test] POST PASS targetInactive=true existingState=unchanged');
 }
 await close(db);
