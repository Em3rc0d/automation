...
if(!['pre','post'].includes(mode)){
  console.error('usage: validate-gate2-execution.js <pre|post>');
  process.exit(2);
}
...
(async()=>{
  ...
  const expectedTenantId=process.env.CASE003_EXPECTED_SMOKE_TENANT_ID||'30000000-0000-0000-0000-000000000000';
  const expected=rows.find(r=>r.invoice_id===expectedInvoiceId);
  if(!expected) throw new Error('expected synthetic smoke invoice not returned');
  if(expected.tenant_id!==expectedTenantId) throw new Error('synthetic tenant mismatch');
