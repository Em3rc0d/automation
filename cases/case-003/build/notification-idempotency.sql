...
create or replace function case003.reserve_due_notification(
  p_tenant_id uuid,
  p_invoice_id uuid,
  p_rule_code text,
  p_canonical_due_date date,
  p_snapshot_id uuid
) returns table(notification_id uuid,idempotency_key text,reserved boolean)
language plpgsql
security invoker
as $$
declare
...
begin
  v_key := encode(digest(
    p_tenant_id::text || '|' || p_invoice_id::text || '|' || p_rule_code || '|' ||
    coalesce(p_canonical_due_date::text,'') || '|' || p_snapshot_id::text,
    'sha256'
  ), 'hex');
  insert into case003.notification_delivery(
    tenant_id,invoice_id,rule_code,canonical_due_date,snapshot_id,idempotency_key,status
  ) values (p_tenant_id,p_invoice_id,p_rule_code,p_canonical_due_date,p_snapshot_id,v_key,'reserved')
  on conflict(idempotency_key) do nothing
  returning id into v_id;
  if v_id is not null then
    return query select v_id,v_key,true;
  else
    return query select n.id,n.idempotency_key,false
      from case003.notification_delivery n where n.idempotency_key=v_key;
  end if;
end $$;
